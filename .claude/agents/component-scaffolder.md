...
### 3. 在 ComponentRegistry.cpp 中添加反射声明 + 注册代码

位置：`Engine/src/Reflection/ComponentRegistry.cpp`

```cpp
ENGINE_COMPONENT(WindZoneComponent, "风场");
ENGINE_PROPERTY(WindZoneComponent, Direction, "方向", Vec3);
ENGINE_PROPERTY(WindZoneComponent, Strength, "强度", Float);

REGISTER_COMPONENT_BEGIN(WindZoneComponent)
    REGISTER_COMPONENT_PROPERTY(WindZoneComponent, Direction)
    REGISTER_COMPONENT_PROPERTY(WindZoneComponent, Strength)
REGISTER_COMPONENT_END(WindZoneComponent)
```

注意：`ENGINE_COMPONENT` / `ENGINE_PROPERTY` 宏现在在 `.cpp` 中声明，不在头文件。
...
```cpp
ENGINE_PROPERTY_EX(MyComp, Speed, "速度", Float,
...
- `PropertyType` 在 `ENGINE_PROPERTY` 宏中写简短形式（`Float`、`Vec3`），非 `PropertyType::Float`
