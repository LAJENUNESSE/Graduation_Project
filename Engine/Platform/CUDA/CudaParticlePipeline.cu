...
__global__ static void EmitKernel(
    ...
    EmitParams    p)
{
    ...
    float z   = cosAngle + (1.0f - cosAngle) * pcg_randf(seed); seed = pcg_hash(seed);
    float phi = pcg_randf(seed) * 6.28318530718f;                seed = pcg_hash(seed);
    ...
    // 从 (0,0,1) → 方向构建切线/副法线
    float ux, uy, uz;
    if (fabsf(dy) < 0.999f) { ux = 0.f; uy = 1.f; uz = 0.f; }
...
__global__ static void RenderArgsKernel(
    CounterData*        counter,
    IndirectDrawCommand* args)
{
...
void LaunchEmit(void* particles, void* deadList, void* counter,
                const EmitParams& params, void* stream)
{
    if (params.emitCount == 0) return;

    uint32_t blocks = (params.emitCount + 63) / 64;
    EmitKernel<<<blocks, 64, 0, static_cast<cudaStream_t>(stream)>>>(
        static_cast<GPUParticle*>(particles),
        static_cast<uint32_t*>(deadList),
        static_cast<CounterData*>(counter),
        params);
}
