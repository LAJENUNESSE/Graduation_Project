...
// 粒子位置 → 网格坐标
__device__ static void PosToCell(float px, float py, float pz, float cellSize,
...
__device__ static void ComputeRigidBodyForce(
    float px, float py, float pz,
    float vx, float vy, float vz,
    float h, float boundaryStiffness, float boundaryDamping,
    const RigidBodyData* d_rigidBody, int rigidBodyCount,
    float& fbx, float& fby, float& fbz)
{
    ...
    for (int rb = 0; rb < rigidBodyCount; rb++)
    {
        ...
        if (rbType < 0.5f) // Box
        {
            ...
            sdf = sqrtf(ox * ox + oy * oy + oz * oz) +
                  fminf(fmaxf(ddx, fmaxf(ddy, ddz)), 0.0f);
            float sx = (lx >= 0.0f) ? 1.0f : -1.0f;
            float sy = (ly >= 0.0f) ? 1.0f : -1.0f;
            float sz = (lz >= 0.0f) ? 1.0f : -1.0f;
            if (ddx > ddy && ddx > ddz)      { lnx = sx; lny = 0.0f; lnz = 0.0f; }
            else if (ddy > ddz)               { lnx = 0.0f; lny = sy; lnz = 0.0f; }
            else                              { lnx = 0.0f; lny = 0.0f; lnz = sz; }
        }
        else // Sphere
        {
            ...
            sdf = lenL - radius;
            if (lenL > 0.001f) { lnx = lx / lenL; lny = ly / lenL; lnz = lz / lenL; }
            else               { lnx = 0.0f; lny = 1.0f; lnz = 0.0f; }
        }
...
__global__ static void HashKernel(
    GPUParticle* particles, uint32_t* d_cellHash, uint32_t* d_cellCount,
    uint32_t aliveCount, int gridSize, float cellSize)
{
...
__global__ static void ScatterKernel(
    uint32_t* d_cellHash, uint32_t* d_cellStart, uint32_t* d_sortedIndices,
    uint32_t aliveCount)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= aliveCount) return;
...
__global__ static void DensityKernel(
    GPUParticle* particles, uint32_t* d_cellStart, uint32_t* d_cellCount,
    uint32_t* d_sortedIndices, SPHParams p)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.aliveCount) return;
...
__global__ static void ForceKernel(
    GPUParticle* particles, uint32_t* d_cellStart, uint32_t* d_cellCount,
    uint32_t* d_sortedIndices, RigidBodyData* d_rigidBody,
    SPHParams p, PCISPHIterParams ip)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.aliveCount) return;
    ...
    // 刚体 SDF 边界力
    float fbx, fby, fbz;
    ComputeRigidBodyForce(px, py, pz, vx_i, vy_i, vz_i,
                          h, ip.boundaryStiffness, ip.boundaryDamping,
                          d_rigidBody, ip.rigidBodyCount,
                          fbx, fby, fbz);
    ...
    if (accelMag > 500.0f) { float s = 500.0f / accelMag; ax *= s; ay *= s; az *= s; }
...
__global__ static void PCISPHInitKernel(
    GPUParticle* particles, PCISPHData* pcisph, uint32_t* d_cellStart,
    uint32_t* d_cellCount, uint32_t* d_sortedIndices, SPHParams p)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.aliveCount) return;
...
__global__ static void PCISPHPredictKernel(
    GPUParticle* particles, PCISPHData* pcisph, float dt, uint32_t aliveCount)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= aliveCount) return;

    // x* = pos + dt * v*
    pcisph[i].predictedPosAndPressure.x = particles[i].posAndLife.x +
                                           dt * pcisph[i].predictedVelAndDensity.x;
    pcisph[i].predictedPosAndPressure.y = particles[i].posAndLife.y +
                                           dt * pcisph[i].predictedVelAndDensity.y;
    pcisph[i].predictedPosAndPressure.z = particles[i].posAndLife.z +
                                           dt * pcisph[i].predictedVelAndDensity.z;
}
...
__global__ static void PCISPHDensityKernel(
    GPUParticle* particles, PCISPHData* pcisph, uint32_t* d_cellStart,
    uint32_t* d_cellCount, uint32_t* d_sortedIndices, SPHParams p, float pcisphDelta)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.aliveCount) return;
...
__global__ static void PCISPHForceKernel(
    GPUParticle* particles, PCISPHData* pcisph, uint32_t* d_cellStart,
    uint32_t* d_cellCount, uint32_t* d_sortedIndices, RigidBodyData* d_rigidBody,
    SPHParams p, PCISPHIterParams ip)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.aliveCount) return;
    ...
    ComputeRigidBodyForce(px, py, pz, predVx, predVy, predVz,
                          h, ip.boundaryStiffness, ip.boundaryDamping,
                          d_rigidBody, ip.rigidBodyCount,
                          fbx, fby, fbz);
...
__global__ static void PCISPHApplyKernel(
    GPUParticle* particles, PCISPHData* pcisph, uint32_t aliveCount)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= aliveCount) return;
...
__global__ static void FluidSimulateKernel(
    GPUParticle* particles, SPHSimulateParams p)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.particleCount) return;
    ...
    if (p.useBoundary)
    {
        if (px < p.boundaryMin[0]) { px = p.boundaryMin[0]; vx =  fabsf(vx) * 0.3f; }
        if (px > p.boundaryMax[0]) { px = p.boundaryMax[0]; vx = -fabsf(vx) * 0.3f; }
        if (py < p.boundaryMin[1]) { py = p.boundaryMin[1]; vy =  fabsf(vy) * 0.3f; }
        if (py > p.boundaryMax[1]) { py = p.boundaryMax[1]; vy = -fabsf(vy) * 0.3f; }
        if (pz < p.boundaryMin[2]) { pz = p.boundaryMin[2]; vz =  fabsf(vz) * 0.3f; }
        if (pz > p.boundaryMax[2]) { pz = p.boundaryMax[2]; vz = -fabsf(vz) * 0.3f; }
    }
...
__global__ static void FluidEmitKernel(
    GPUParticle* particles, SPHEmitParams p)
{
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= (uint32_t)p.particleCount) return;
