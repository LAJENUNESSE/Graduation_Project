...
namespace Engine
{
    ...
    private:
        ...
        // validation 开关：Debug 构建默认开启、Release 关闭；环境变量
        // ENGINE_VULKAN_VALIDATION（=1 强制开 / =0 强制关）可覆盖，供排查时使用。
        static bool ResolveValidationEnabled();
        static bool s_EnableValidation;
    };
