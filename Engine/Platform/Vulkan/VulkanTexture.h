...
namespace Engine
{
    ...
    public:
        ...
        uint32_t GetWidth() const override { return m_FaceSize; }
        uint32_t GetHeight() const override { return m_FaceSize; }
        uint32_t GetRendererID() const override { return 0; }
    ...
    private:
        void CreateImage(uint32_t faceSize, VkFormat format);
        void CreateCubeView(VkFormat format);
        void CreateSampler();
        void UploadFaces(const void* faceContiguousRGBA8, uint32_t faceSize);
        void TransitionLayoutCube(VkImageLayout oldLayout, VkImageLayout newLayout);
        void Destroy();
