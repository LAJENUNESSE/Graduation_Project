    ...
    public:
        ...
        template <typename T, typename... Args> T& AddComponent(Args&&... args)
        {
            ...
            if (Log::GetCoreLogger())
                ENGINE_CORE_TRACE("[ComponentLifecycle] Add '{0}' -> entity {1}", typeid(T).name(),
                                  static_cast<uint32_t>(m_EntityHandle));
            return component;
        }
        ...
        template <typename T, typename... Args> T& AddOrReplaceComponent(Args&&... args)
        {
            ...
            if (Log::GetCoreLogger())
                ENGINE_CORE_TRACE("[ComponentLifecycle] AddOrReplace '{0}' -> entity {1}", typeid(T).name(),
                                  static_cast<uint32_t>(m_EntityHandle));
            return component;
        }
        ...
        template <typename T> void RemoveComponent()
        {
            ...
            if (Log::GetCoreLogger())
                ENGINE_CORE_TRACE("[ComponentLifecycle] Remove '{0}' <- entity {1}", typeid(T).name(),
                                  static_cast<uint32_t>(m_EntityHandle));
        }
