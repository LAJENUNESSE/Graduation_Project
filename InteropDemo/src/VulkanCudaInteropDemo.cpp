...
#include "Engine/Platform/CUDA/VulkanExternalInterop.h"
...
namespace
{
    ...
    using Engine::CudaInterop::CudaExternalInteropContext;
    using Engine::CudaInterop::VulkanExternalExport;
    ...
    class DemoCudaExternalInteropRunner
    {
    public:
        DemoCudaExternalInteropRunner() = default;
        ~DemoCudaExternalInteropRunner() { Destroy(); }

        void Initialize(OwnedInteropHandle memoryHandle,
                        OwnedInteropHandle semaphoreHandle,
                        size_t             exportMemorySize)
        {
            CudaExternalInteropContext::InitDesc initDesc{};
            initDesc.MemoryHandle     = memoryHandle;
            initDesc.SemaphoreHandle  = semaphoreHandle;
            initDesc.SharedBufferSize = exportMemorySize;
            initDesc.HandleType       = GetDefaultExternalHandleType();

            m_Interop.Initialize(initDesc);
            m_DeviceVertices = static_cast<InteropDemo::CudaParticleVertex*>(m_Interop.GetMappedDevicePointer());
                                ...
                                float                         timeSeconds)
        {
            ...
            const cudaStream_t stream = static_cast<cudaStream_t>(m_Interop.GetStream());

            CheckCuda(cudaEventRecord(m_WaitStart, stream), "cudaEventRecord(waitStart)");
            m_Interop.WaitTimelineValue(syncValues.WaitVulkanValue);
            CheckCuda(cudaEventRecord(m_WaitEnd, stream), "cudaEventRecord(waitEnd)");

            CheckCuda(cudaEventRecord(m_KernelStart, stream), "cudaEventRecord(kernelStart)");
            ...
            CheckCuda(cudaEventRecord(m_KernelEnd, stream), "cudaEventRecord(kernelEnd)");

            CheckCuda(cudaEventRecord(m_SignalStart, stream), "cudaEventRecord(signalStart)");
            m_Interop.SignalTimelineValue(syncValues.CudaSignalValue);
            CheckCuda(cudaEventRecord(m_SignalEnd, stream), "cudaEventRecord(signalEnd)");
    ...
    private:
        CudaExternalInteropContext       m_Interop;
    ...
    private:
        ...
        DemoCudaExternalInteropRunner m_CudaInterop;
    };
