...
StructuredBuffer<GPURigidBody>    rigidBodies     : register(t2);
StructuredBuffer<GPUMeshSDFBody>  meshSDFBodies   : register(t3);
StructuredBuffer<float>           meshSDFVoxels   : register(t4);
StructuredBuffer<uint>            cellStart       : register(t5);
StructuredBuffer<uint>            cellCount       : register(t6);
StructuredBuffer<uint>            sortedIndices   : register(t7);
...
void CSMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    float3 posI = (u_UsePredictedPos != 0)
        ...
        for (int dy = -1; dy <= 1; dy++)
        {
            ...
            for (int dx = -1; dx <= 1; dx++)
            {
                ...
                for (uint s = cBegin; s < cEnd; s++)
                {
                    ...
                    float3 posJ = (u_UsePredictedPos != 0)
    ...
    for (int rb = 0; rb < u_RigidBodyCount; rb++)
    {
        ...
        if (sdf < u_SmoothingRadius)
        {
            ...
            float3 velRel = pcisphData[gid].predictedVelAndDensity.xyz - rbVel;
            float penetration = max(0.0f, u_SmoothingRadius - sdf);
    ...
    for (int mb = 0; mb < u_MeshSDFCount; mb++)
    {
        ...
        if (sdf < u_SmoothingRadius)
        {
            ...
            float3 velRel = pcisphData[gid].predictedVelAndDensity.xyz - mbVel;
            float penetration = max(0.0f, u_SmoothingRadius - sdf);
