...
- 重新上传同一初始状态，并以固定时间步执行 120 个不计时验证帧，保证三个后端在相同模拟步数后比较；
...
2026-07-19 的三后端短矩阵校准采用 1000 粒子、6 次 PCISPH 迭代、固定 120 个验证步。六组实验的 Alive Count、有限值和边界检查均通过；相对 OpenGL 基线，平均密度最大相对偏差为 0.0261%（CUDA/PCISPH）。据此在正式矩阵前冻结平均密度相对容差为 **0.1%（0.001）**。该容差约为校准最大偏差的 3.8 倍，用于容纳不同着色器编译器和浮点执行顺序造成的非逐位一致，同时仍能拒绝此前由压力语义或跨帧网格竞争造成的百分比级算法偏差。

状态 Hash 仅用于复现实验和定位变化，不作为跨 API 的逐位一致性门槛。正式矩阵中任一组超过 0.1% 容差时，该组标记为正确性失败且不计算加速比；禁止在看到正式结果后再放宽容差。
